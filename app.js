...
/* ---------- Data Film ---------- */
const POSTER_WORKER = "https://idflix-bot1.firman-uke29.workers.dev";

const normalize = (id,f) => {
  const t=f.title||"Tanpa Judul";
  const g=Array.isArray(f.genre)
    ? f.genre
    : String(f.genre||"").split(",").map(x=>x.trim()).filter(Boolean);

  /*
   * Poster:
   * 1. Gunakan poster lama jika tersedia.
   * 2. Jika ada posterFileId dari Telegram, gunakan proxy Worker.
   * 3. Jika tidak ada keduanya, gunakan placeholder.
   */
  const poster =
    f.poster ||
    (
   ...
   * }
   *
   * Tetap mendukung format lama berupa string.
   */
  ...
  const videos=Object.entries(rawVideos).map(([quality,v])=>({
    ...
    videoUrl:typeof v==="string"
      ? v
      : String(v?.videoUrl||v?.url||""),
    telegramFileId:v?.telegramFileId||v?.fileId||""
  })).filter(v=>v.videoUrl);

  /*
   * Kompatibilitas data lama:
   * jika tidak ada videos[], gunakan videoUrl lama.
   */
  if(!videos.length && f.videoUrl){
    videos.push({
      quality:String(f.quality||"Default"),
  ...
  return {
    ...
    title:t,
    year:f.year||"",
    genre:g.length?g:["Lainnya"],
    duration:f.duration||"",
    rating:Number(f.rating)||0,
    description:f.description||"",
    videoUrl:videos[0]?.videoUrl||f.videoUrl||"",
    videos,
    addedAt:f.addedAt||0,
    poster,
    backdrop:f.backdrop||art(t,"#3a3a48","#0f0f16",1280,720)
  };
};

// Firebase Realtime Database (node "movies"), diisi oleh Telegram Bot.
// Tanpa konfigurasi -> pakai data dummy.
async function loadFilms(){
  const c = window.FIREBASE_CONFIG;

  if(!window.firebase || !c || /PROJECT/.test(c.databaseURL||"PROJECT")) {
    return films;
  }
...
const section = (title,body,link) =>
  `<section class="sec"><div class="sec-head"><h2>${title}</h2>${link?`<a href="${link}">Lihat Semua ${ic("chev")}</a>`:""}</div>${body}</section>`;

const row = (list,opt) =>
  `<div class="row">${list.map(f=>card(f,opt)).join("")}</div>`;

const grid = list =>
  `<div class="grid">${list.map(f=>card(f)).join("")}</div>`;

const chips = (active,base) =>
  `<div class="chips">${["Semua",...GENRES].map(g=>`<a class="chip ${(active||"Semua")===g?"on":""}" href="${base}${g==="Semua"?"":"?g="+g}">${g}</a>`).join("")}</div>`;

const empty = (icon,t,d) =>
  `<div class="empty">${ic(icon)}<b>${t}</b>${d}</div>`;

const meta = f =>
  `<div class="meta"><span>${f.year}</span><i></i><span>${f.genre.join(" • ")}</span><i></i><span>${f.duration}</span><i></i><span>★ ${f.rating}</span></div>`;

const favBtn = f =>
  `<button class="btn ghost ${favs.includes(f.id)?"on":""}" data-fav="${f.id}">${ic(favs.includes(f.id)?"check":"plus")} ${favs.includes(f.id)?"Di Favorit":"Tambah ke Favorit"}</button>`;
...
function home(){
  if(!films.length)
    return empty("film","Belum ada film","Kirim video ke Telegram Bot untuk menambahkan film.");
...
function heroSlide(f){
  return `<img class="hero-bg" src="${f.backdrop}" alt=""><div class="hero-body"><span class="badge">FILM TERPOPULER</span><h1>${esc(f.title)}</h1>${meta(f)}<p>${esc(f.description)}</p><div class="actions"><a class="btn primary" href="#/watch/${f.id}">${ic("play")} Tonton Sekarang</a>${favBtn(f)}</div></div>`;
}
...
function startHero(top){
  ...
  const show = n => {
    ...
    const d=h.querySelector(".dots");
    h.innerHTML=heroSlide(top[i]);
    h.appendChild(d);
...
const movies = q => {
  const g=q.get("g");
  const list=g?films.filter(f=>f.genre.includes(g)):films;
...
const categories = () =>
  `<h1 class="page-title">Kategori</h1><section class="sec"><div class="tiles">${GENRES.map((g,i)=>`<a class="tile" style="background:linear-gradient(135deg,${tileColors[i%9]},#0d0d14)" href="#/movies?g=${g}">${g}<small>${films.filter(f=>f.genre.includes(g)).length} film</small></a>`).join("")}</div></section>`;
...
const resultsHTML = (term,res) =>
  !term
    ? `${empty("search","Mulai mencari","Ketik judul, genre, atau tahun rilis film.")}<div class="sec"><div class="sec-head"><h2>Telusuri genre</h2></div>${chips("","#/movies")}</div>`
    : res.length
      ? grid(res)
      : empty("search","Tidak ada hasil",`Tidak ada film yang cocok dengan “${esc(term)}”. Coba kata kunci lain.`);

function profile(){
  const fl = films.filter(f=>favs.includes(f.id));
  const cont = films.filter(f=>progress[f.id]>0);
...
function detail(id){
  ...
  const rel=films
    .filter(x=>x.id!==id&&x.genre.some(g=>f.genre.includes(g)))
    .slice(0,8);
...
function qualityLabel(q){
  return String(q).toLowerCase().endsWith("p")?String(q):String(q);
}
...
function watch(id){
  ...
  /*
   * Video baru:
   * f.videos berasal dari Firebase movies/{id}/videos
   * dan setiap quality memiliki videoUrl langsung.
   *
   * Fallback:
   * f.videoUrl untuk data lama.
   */
...
const notFound = () =>
  empty("film","Film tidak ditemukan",`<br><a class="btn primary" href="#/">Kembali ke Beranda</a>`);
...
function router(){
  ...
  if(seg!=="search") ts.value = "";
  else ts.value=q.get("q")||"";

  if(seg==="watch") bindPlayer(byId(arg));

  if(seg==="search" && matchMedia("(max-width:639px)").matches && !q.get("q"))
    $("#pageSearch")?.focus();
}
...
function bindPlayer(f){
  ...
  /*
   * Ganti sumber video tanpa bergantung pada provider.
   * URL dapat berasal dari hosting video mana pun selama browser
   * dapat memutar URL tersebut sebagai HTML5 video.
   */
  const setSource=(url,quality,autoplay=false)=>{
    ...
    v.addEventListener("loadedmetadata",function once(){
      ...
      if(saved>0&&saved<.97)
        v.currentTime=saved*v.duration;
      else if(oldTime>0&&Number.isFinite(oldTime))
        v.currentTime=Math.min(oldTime,v.duration||oldTime);
  ...
  buttons.forEach(btn=>btn.addEventListener("click",()=>{
    ...
    if(oldTime>0){
      v.addEventListener("loadedmetadata",()=>{
        if(v.duration)
          v.currentTime=Math.min(oldTime,v.duration);
      },{once:true});
    }
  }));
  ...
  v.addEventListener("timeupdate",()=>{
    ...
    const active=
      document.querySelector("[data-quality].on")?.dataset.quality
      ||f.videos?.[0]?.quality
      ||"default";
...
function liveSearch(val){
  ...
  const res=term
    ?films.filter(f=>(f.title+" "+f.genre.join(" ")+" "+f.year).toLowerCase().includes(term))
    :[];

  $("#results").innerHTML = resultsHTML(term,res);

  document.querySelector(".page-title").textContent =
    term?`Hasil untuk “${val}”`:"Cari Film";
}

document.addEventListener("input",e=>{
  if(e.target.id==="topSearch"||e.target.id==="pageSearch"){
    ...
    if(o&&o!==e.target) o.value=v;
    if(m&&m!==e.target) m.value=v;
  }
});

document.addEventListener("click",e=>{
  const fb=e.target.closest("[data-fav]");
  const dt=e.target.closest("[data-slide]");
  ...
  if(fb){
    ...
    store.set("idflix:favs",favs);

    const f=byId(id);

    if(f)
      fb.outerHTML=favBtn(f);

    return;
  }

  if(dt)
    window.__slide(+dt.dataset.slide);
});
